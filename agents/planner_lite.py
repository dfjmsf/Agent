...
- 输出仅给用户看，不传递给 Manager

与 Manager 的区别：
- PlannerLite: Markdown 预览（给人看） → 便宜、快速
- Manager: JSON project_spec（给机器看） → 精确、贵
"""
import os
import json
import logging
...
class PlannerLiteAgent:
    """轻量级规划组 Agent — 生成 plan.md 供用户预览确认"""

    def __init__(self):
        self.model = os.getenv("MODEL_PLANNER_LITE", "deepseek-chat")

    def generate_plan(self, structured_req: dict) -> str:
        """
        从标准化需求生成 plan.md（Markdown 文档）。

        Args:
            structured_req: PM 标准化后的需求 JSON
                {summary, core_features, implied_requirements,
        ...
        try:
            ...
            # 降级：用结构化需求直接拼凑一个简单方案
            return self._fallback_plan(structured_req)
    ...
    def _fallback_plan(req: dict) -> str:
        ...
        lines = [f"# {summary} 技术方案", "", "## 技术选型"]
        lines.append(f"- 后端：{tech.get('backend', 'Flask')}")
        lines.append(f"- 数据库：{tech.get('database', 'SQLite')}")
        lines.append(f"- 前端：{tech.get('frontend', 'Jinja2 SSR')}")

        if defaults:
            for d in defaults:
                lines.append(f"  - ⚠️ {d['field']}：{d['value']}（{d['reason']}）")

        lines.extend(["", "## 功能清单"])
        for f in features:
            lines.append(f"- [ ] {f}")

        lines.extend(["", "## 预估规模", "待评估", ""])
        return "\n".join(lines)
