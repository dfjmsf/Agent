...
## 11. 表单取值安全铁律（违反 = 500 Internal Server Error）

> ⚠️ **`request.form.get()` 返回的是字符串或 None！必须先验证非空，再做类型转换！**

```python
# ❌ 致命错误：先转换再验证 → amount 为空时 float(None) 直接 500！
amount = float(request.form.get('amount'))  # ← TypeError: float(None)
if not amount:
    flash('金额不能为空')

# ✅ 正确：先取值验证，再在 try-except 中转换
...
## 12. row_factory 铁律（违反 = Jinja2 UndefinedError = 500 崩溃）

> ⚠️ **致命铁律：每一个 `sqlite3.connect()` 调用后，必须立刻设置 `conn.row_factory = sqlite3.Row`！**

- 不设置 → `fetchall()` 返回 tuple → 模板 `{{ expense.amount }}` 报错 `'tuple' object has no attribute` → 500！
- **推荐**：封装 `get_db()` 工具函数统一设置，所有查询函数都用 `get_db()` 获取连接。
...
# ❌ 致命错误：遗漏 row_factory
def get_expenses():
    conn = sqlite3.connect(DB_PATH)  # 没有 row_factory → 返回 tuple → 模板崩溃！
