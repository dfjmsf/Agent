...
{{
  ...
  "tasks": [
    {{
      ...
      "task_type": "new_file",
      "description": "创建新模块文件并实现功能",
      "dependencies": [],
      "write_targets": ["new_module.py"]
    }},
    {{
      "task_id": "ext_weld_1",
      "target_file": "app.py",
      "task_type": "weld",
      "draft_action": "modify",
      "description": "在 app.py 中 import 并注册新 blueprint",
      "dependencies": ["ext_1"],
      "write_targets": ["app.py"]
    }},
    {{
      "task_id": "ext_weld_nav",
      "target_file": "templates/base.html",
      "task_type": "weld",
      "draft_action": "modify",
      "description": "在 base.html 导航栏中添加指向新模块页面的链接（如 /stats/category、/stats/monthly）",
      "dependencies": ["ext_1"],
      "write_targets": ["templates/base.html"]
    }}
  ]
}}
"""
...
6. 不要重复备忘录中已有的内容"""
    ...
    # ---- PM Tool Calling 路由工具定义 (v4.0 — 纯 LLM 意图引擎) ----
    # 4 个工具，零正则，PM 通过引导式交互让用户无感给出准确路由
    PM_ROUTE_TOOLS = [
        {
            "type": "function",
            "function": {
                ...
                "description": (
                    "当用户明确要求执行操作、确认当前方案、或继续下一阶段时调用。\n"
                    "场景：\n"
                    "- 用户描述新项目需求（项目不存在时）-> mode=create\n"
                    "- 用户报告已有项目的 bug/问题/渲染异常/请求修复 -> mode=modify\n"
                    "- 用户要修改已有项目的某个功能/页面/样式/行为 -> mode=modify\n"
                    "- 用户要给已有项目新增功能或调整现有功能的实现方式 -> mode=modify\n"
                    "- 有待确认方案且用户表示肯定 -> mode 匹配当前待执行内容\n"
                    "- 用户要求继续修复或进入下一阶段 -> mode=continue\n"
                    "- 用户要撤销/回滚 -> mode=rollback\n"
                    "- 用户要审查代码 -> mode=audit"
                ),
                "parameters": {
                    "type": "object",
                    "properties": {
                        "mode": {
                            ...
                            "description": (
                                "create=从零新建一个不存在的项目; "
                                "modify=对已有项目做任何改动(bug修复/功能调整/样式修改/功能增强/实现方式变更等); "
                                "continue=继续修复/继续下一阶段; "
                                "rollback=回滚; "
                                "audit=审查代码质量"
                            ),
                        },
        ...
        },
        {
            ...
            "function": {
                ...
                "description": "用户想修改当前方案的某些细节（如换技术栈、调整功能、移除某个模块）。仅适用于有待确认方案的阶段。",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "revision_summary": {
        ...
        },
        {
            ...
            "function": {
                "name": "reply_to_chat",
                "description": "非操作性消息：闲聊、问答、打招呼、用户说'等等'、'让我想想'、感谢等。",
                "parameters": {
                    "type": "object",
                    "properties": {},
                },
        ...
        },
        {
            ...
            "function": {
                ...
                "description": "用户意图模糊，无法判断属于哪个操作时追问。绝不猜测，宁可追问。",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "question": {
...
1. 【最高优先】已有项目 + 用户报告问题/bug/渲染异常/请求修复 → execute_project_task(mode=modify)
   关键词："无法渲染"、"报错"、"500"、"打不开"、"显示异常"、"有问题"、"修复"、"修一下"、"bug"
   注意：用户描述已有功能的问题时，这是修改请求，不是新建项目！

2. 用户对当前展示的方案/修改预览表达肯定（"可以"、"开始"、"没问题"、"做吧"、"继续"、"好的"）→ execute_project_task，mode 匹配当前待执行内容
3. 用户说"继续修复"或"接着修" → execute_project_task(mode=continue)
4. 用户要撤销/回滚 → execute_project_task(mode=rollback)
5. 用户要审查代码质量 → execute_project_task(mode=audit)
6. 用户对方案提出具体修改意见 → route_to_revise_plan
7. 用户意图模糊（你不确定用户想做什么）→ ask_for_clarification
8. 非操作性消息（闲聊/问候/感谢） → reply_to_chat
9. 用户描述新项目需求（项目不存在或为空） → execute_project_task(mode=create)

【create vs modify 判断铁律】
- 项目已存在且有文件 -> 默认走 modify，除非用户明确要求"重新创建""从头开始""推翻重做"
- 用户描述的是已有功能的问题（如 "/edit 无法渲染"、"删除接口报 500"） -> 这是 bug 修复，走 modify
- 用户要求修改某个已有功能的实现方式（如"把输入框改成下拉选择"、"把颜色换成蓝色"） -> 这是功能调整，走 modify
- 用户要求给已有项目新增一个功能（如"加上搜索功能"、"增加导出功能"） -> 这也是走 modify（不是创建新项目！）
- 只有当项目不存在（project_status 显示"不存在或为空项目"）且用户描述全新需求时才走 create
- 绝对禁止对已有项目使用 create！create 会导致从零重建，覆盖所有已有代码！

【上下文感知 — 关键】
- 如果 project_status 标注"有待确认方案"，用户的肯定回复 → execute_project_task(mode=create)
- 如果 project_status 标注"有待确认的修改方案"，用户肯定 → execute_project_task(mode=modify)
- 如果 project_status 标注"Phase N+1 待确认"，用户说"继续" → execute_project_task(mode=continue)
- 如果 project_status 标注"有未修复的问题"，用户说"修复" → execute_project_task(mode=continue)"""
