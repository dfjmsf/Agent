...
{route_hint}"""
    ...
    # ---- PM Tool Calling 路由工具定义 (v2.6) ----
    PM_ROUTE_TOOLS = [
        {
            "type": "function",
            "function": {
                "name": "route_to_create",
                "description": "用户想创建全新项目或从零开始开发新功能时调用。关键词：做一个、创建、新建、开发。",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "user_requirement": {
        ...
        },
        {
            ...
            "function": {
                "name": "route_to_patch",
                "description": "用户想修改已有项目中的某个部分时调用（改颜色、加功能、删按钮、修复bug等）。前提是项目已存在，且用户的修改目标明确。",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "modification_summary": {
                            "type": "string",
                            "description": "用户想做的修改的一句话摘要"
                        }
                    },
                    "required": ["modification_summary"]
                }
            }
        },
        {
            "type": "function",
            "function": {
                "name": "route_to_rollback",
                "description": "用户想撤销、恢复、改回去之前的修改时调用。关键词：撤销、改回去、回滚、恢复上一版、Round N。",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "rollback_hint": {
                            "type": "string",
                            "description": "用户想回退的目标描述（如'上一次修改'、'Round 2'等）"
                        }
                    },
        ...
        },
        {
            ...
            "function": {
                "name": "reply_to_chat",
                "description": "用户在闲聊、问候、提问、感谢、技术咨询、或对话跟进时调用。",
                "parameters": {
                    "type": "object",
                    "properties": {},
                }
        ...
        },
        {
            ...
            "function": {
                ...
                "description": "当用户需求模糊、缺乏明确的操作对象或操作动作时必须调用此工具。绝对不要猜测用户意图，宁可追问也不强行路由。",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "question": {
                            "type": "string",
                            "description": "向用户提出的澄清问题，应该简洁友好"
                        },
                        "options": {
                            "type": "array",
                            "items": {"type": "string"},
                            "description": "可选：提供 2-3 个具体方向供用户参考选择"
                        }
                    },
                    "required": ["question"]
                }
    ...
    PM_ROUTE_SYSTEM = """你是 ASTrea 的项目经理(PM)。根据用户消息和项目状态，选择合适的工具进行路由。

【项目状态】
{project_status}

【路由纪律】
- 用户说"做一个/创建/新建XX" → route_to_create
- 用户说"改/修改/加个/删掉XX" + 项目已存在 → route_to_patch
- 用户说"撤销/改回去/回滚/恢复" → route_to_rollback
- 用户闲聊/问问题/打招呼/感谢 → reply_to_chat
- 用户需求模糊（缺少操作对象或动作） → ask_for_clarification（绝对不猜）

【模糊判定标准】
以下情况必须调用 ask_for_clarification：
- "改好看点" → 缺少具体对象和方向
- "那个跑不通" → 缺少具体错误和文件
- "弄一下" → 完全不明确
提供的选项应具体、可操作，帮助用户快速做出选择。"""
