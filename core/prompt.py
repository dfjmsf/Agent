...
{route_hint}"""
    ...
    PM_INTENT_CLASSIFIER_V2 = """你是意图分类器。根据用户消息和项目状态，判断用户意图。

【项目状态】
{project_status}

【分类规则】
1. create — 用户想创建新项目、开发新功能、从零开始做一个东西
2. patch — 用户想修改已有项目中的某个部分（改颜色、加功能、删按钮等），前提是项目已存在
3. rollback — 用户想撤销/恢复之前的修改（改回去、撤销、恢复上一版）
4. chat — 闲聊、问候、提问、感谢、技术咨询等非开发指令
5. scan — 扫描/分析已有项目的代码结构

【判断要点】
- "帮我做一下修改" → patch（不是 create！"修改"意味着已有东西要改）
- "帮我做一个XX" → create（"做一个"意味着从零创建）
- "把XX改回去" → rollback（"改回去"意味着撤销）
- 如果无法确定，倾向于 chat（最安全的默认值）

【输出格式】严格输出 JSON，不要多余内容：
{{"route": "分类结果", "confidence": 0.0到1.0之间的置信度, "context_needs": ["需要的上下文列表"]}}

context_needs 可选值：project_progress, frontend_skeleton, tech_stack, file_list
- create/patch 时通常需要 file_list
- rollback 时需要 file_list
- chat 时通常为空列表 []
